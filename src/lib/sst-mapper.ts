...
export function buildSSTPayload(
  ...
  // otherwise 0 (no king post = no vertical height to report).
  ...
  // Use hangerAngle already assigned during computeCarriedTrussGeometry()
  // which maps each LG*T entry in order to each CarriedTruss — no re-lookup needed.
  const hangerAngle = carried.hangerAngle ?? 90;
